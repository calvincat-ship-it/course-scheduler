...
let settingsOpen = {};        // v12.42 設定頁各摺疊區展開狀態（runtime，key＝data-sg；重繪後保持）
...
let reschedOpenId = null;   // v12.18 調課頁：null＝清單；'new'＝新增編輯器
let reschedDraft = null;    // v12.22 教師導向草稿 {teacherId,startDate,endDate,note,swaps:[{a,b,seedClassId,classIds}],picking:{seedClassId,src}|null}
...
// 對調範圍＝來源班在 a、b 兩時段所屬「協同群組」的聯集（含來源班本身）。單班課→只含該班。
function swapScope(seedClassId, a, b) {
  const set = new Set([seedClassId]); const c = classById(seedClassId); if (!c) return [seedClassId];
  const add = (day, period) => { const sid = state.slots[slotKey(seedClassId, day, period)]; if (sid) classCoteachPartners(c, sid).forEach(id => set.add(id)); };
  add(a.day, a.period); add(b.day, b.period);
  return [...set];
}
// 把指定班級集合的兩個時段內容（含分節老師）逐班對調（就地改 state；呼叫端負責 snapshot/restore）
function applySwapToScope(classIds, a, b) {
  classIds.forEach(cid => {
    const kA = slotKey(cid, a.day, a.period), kB = slotKey(cid, b.day, b.period);
...
// 對調是否合法（指定 class scope a↔b）。回 {ok:true,warnings} 或 {ok:false, reason}。
function swapLegal(classIds, a, b) {
  if (a.day === b.day && a.period === b.period) return { ok: false, reason: '同一格' };
  const snap = snapshotGrid();
  try {
    const before = new Set(Object.keys(computeConflicts()));
    const affected = [];
    classIds.forEach(cid => { affected.push(slotKey(cid, a.day, a.period), slotKey(cid, b.day, b.period)); });
    applySwapToScope(classIds, a, b);
...
// 日期區間 [start,end] 涵蓋的星期幾集合(1..5)。任一端缺→null。
function rangeWeekdays(startDate, endDate) {
  if (!startDate || !endDate) return null;
  const start = new Date(startDate + 'T00:00:00'), end = new Date(endDate + 'T00:00:00');
  if (isNaN(start.getTime()) || isNaN(end.getTime()) || start > end) return null;
  const span = Math.round((end - start) / 86400000); const set = new Set();
  for (let i = 0; i <= span && set.size < 5; i++) { const wd = new Date(start.getTime() + i * 86400000).getDay(); if (wd >= 1 && wd <= 5) set.add(wd); }
  return set;
}
/* ---------- 日期式調課：日期工具 ---------- */
// 區間內所有上課日（週一~五）的日期字串陣列
function schoolDatesInRange(startDate, endDate) {
  if (!startDate || !endDate) return [];
  const start = new Date(startDate + 'T00:00:00'), end = new Date(endDate + 'T00:00:00');
  if (isNaN(start.getTime()) || isNaN(end.getTime()) || start > end) return [];
  const out = [];
  for (const dt = new Date(start); dt <= end; dt.setDate(dt.getDate() + 1)) { const wd = dt.getDay(); if (wd >= 1 && wd <= 5) out.push(localDateStr(dt)); }
  return out;
}
const weekdayOf = dateStr => { const dt = new Date(dateStr + 'T00:00:00'); return isNaN(dt.getTime()) ? 0 : dt.getDay(); };   // 1=一..5=五
// 該日期所屬週的週一日期字串（週分組鍵）
function weekMondayOf(dateStr) { const dt = new Date(dateStr + 'T00:00:00'); const wd = dt.getDay(); dt.setDate(dt.getDate() - ((wd + 6) % 7)); return localDateStr(dt); }
const fmtMD = dateStr => { const [, m, d] = (dateStr || '').split('-'); return m ? `${+m}/${+d}` : ''; };
// 把編輯器表單值（備註）同步進草稿，避免 re-render 遺失
function reschedSyncForm() { if (!reschedDraft) return; const n = document.getElementById('reschedNote'); if (n) reschedDraft.note = n.value; }
/* ---------- 日期式調課：來源枚舉／目標合法性 ---------- */
const addDays = (dateStr, n) => localDateStr(new Date(new Date(dateStr + 'T00:00:00').getTime() + n * 86400000));
// 某 slotKey 在某「日期」是否已有代課指派（回 subId 或 null）
function substSubForDate(key, date) {
  const [, , p] = key.split('|');
  for (const rec of (state.substitutions || [])) {
    if (!rec.startDate || !rec.endDate || date < rec.startDate || date > rec.endDate) continue;
...
function reschedSourceLessons(teacherId, leaveStart, leaveEnd, priorSwaps) {
  ...
  for (const date of schoolDatesInRange(leaveStart, leaveEnd)) {
    ...
    for (const rec of (state.substitutions || [])) {
      ...
      if (!rec.startDate || !rec.endDate || date < rec.startDate || date > rec.endDate) continue;
      const wks = substWeeks(rec); const wIdx = wks.length > 1 ? (wks.find(w => date >= w.start && date <= w.end) || {}).idx : null;
      const consider = k => {
        const i = k.indexOf('|'); const isOvr = /^\d+\|/.test(k) && k.split('|').length === 4;
        const key = isOvr ? k.slice(i + 1) : k;
        if (seen.has(key)) return; seen.add(key);
        const [classId, dStr, p] = key.split('|'); if (Number(dStr) !== wd || !substPeriodOnLeave(rec, p)) return;
        if (substEffectiveSub(rec, (wIdx == null ? null : wIdx), key) !== teacherId) return;
        const sid = state.slots[key]; if (!sid) return;
        lessons.push({ key, classId, day: wd, period: p, sid, date, isCover: true, coverRecId: rec.id, coverFor: rec.absentTeacherId });
      };
      Object.keys(rec.assignments || {}).forEach(consider);
      Object.keys(rec.weekOverrides || {}).forEach(consider);
    }
    lessons.sort((a, b) => (pIdx(a.period) - pIdx(b.period)) || ((classById(a.classId) || {}).name || '').localeCompare((classById(b.classId) || {}).name || '', 'zh-Hant'));
    if (lessons.length) out.push({ date, wd, lessons });
  }
  return out;
}
...
/* ---------- 1e 代課↔調課互相參照 ---------- */
// 某 swap 影響的班級 id（新記錄存 classIds＝協同群組快照；舊 grade 式記錄 fallback 整年級）
function swapClassIds(sw, rec) {
  if (Array.isArray(sw.classIds) && sw.classIds.length) return sw.classIds;
  return rec && rec.gradeId ? gradeClassList(rec.gradeId).map(c => c.id) : [];
}
// 反向：某班某位置(day,period)於代課日期區間 [start,end] 內是否被日期式調課移走（該班在 swap 範圍）→ 回 {day,period,date} 或 null。
function reschedMovedTarget(classId, day, period, startDate, endDate) {
  ...
  for (const r of (state.reschedules || [])) for (const sw of (r.swaps || [])) {
    if (!swapClassIds(sw, r).includes(classId)) continue;
    if (sw.aDate >= startDate && sw.aDate <= endDate && sw.a.day === dN && String(sw.a.period) === pS) return { day: sw.b.day, period: sw.b.period, date: sw.bDate };
    if (sw.bDate >= startDate && sw.bDate <= endDate && sw.b.day === dN && String(sw.b.period) === pS) return { day: sw.a.day, period: sw.a.period, date: sw.aDate };
  }
  return null;
}
...
function substReschedConflicts(rec) {
  ...
  for (const key in state.slots) {
    ...
    if (mv) out.push({ className: (classById(classId) || {}).name || '', cell: DAY_LABELS[day] + periodLabel(p), subject: subjectName(state.slots[key]), movedTo: fmtMD(mv.date) + ' ' + DAY_LABELS[mv.day] + periodLabel(mv.period) });
  }
  return out;
}
...
function reschedAffectedClassIds(rec) {
  const set = new Set();
  (rec.swaps || []).forEach(sw => swapClassIds(sw, rec).forEach(id => set.add(id)));
  return state.classes.filter(c => set.has(c.id)).map(c => c.id);
}
// 受此調課影響的教師 id（教到任一對調端點課的老師；依教師名單順序）
function reschedAffectedTeacherIds(rec) {
  const ids = new Set();
  (rec.swaps || []).forEach(sw => swapClassIds(sw, rec).forEach(cid => [sw.a, sw.b].forEach(pos => {
    slotAssignments(slotKey(cid, pos.day, pos.period)).forEach(x => ids.add(x.teacherId));
  })));
  return state.teachers.filter(t => ids.has(t.id)).map(t => t.id);
}
// 此調課涉及的週（回 [週一日期字串]，排序）
function reschedWeeks(rec) {
  const set = new Set();
  (rec.swaps || []).forEach(sw => { if (sw.aDate) set.add(weekMondayOf(sw.aDate)); if (sw.bDate) set.add(weekMondayOf(sw.bDate)); });
  return [...set].sort();
}
// 某週上課日 [{date,wd}]（週一~五）
function weekSchoolDays(monday) {
  const out = []; const base = new Date(monday + 'T00:00:00');
  for (let i = 0; i < 5; i++) { const dt = new Date(base.getTime() + i * 86400000); out.push({ date: localDateStr(dt), wd: dt.getDay() }); }
  return out;
}
// 調課後·某班·某週課表（欄＝該週日期；套日期式對調＋當日代課；對調格標🔀）
function reschedClassWeekHTML(classId, rec, monday) {
  const wdays = weekSchoolDays(monday); const c = classById(classId); const g = classGrade(c);
  let html = `<div class="print-only" style="text-align:center;font-weight:700;font-size:15px;margin-bottom:6px">${esc((c || {}).name || '')} 課表（調課後）</div>`;
  html += `<table class="timetable"><thead><tr><th class="period-th">節次</th>${wdays.map(w => `<th>${DAY_LABELS[w.wd]}<small>${esc(fmtMD(w.date))}</small></th>`).join('')}</tr></thead><tbody>`;
  for (const p of state.settings.periods) {
    if (p.isBreak) { html += `<tr class="break-row"><td colspan="${wdays.length + 1}">${esc(p.label)}</td></tr>`; continue; }
    html += `<tr><td class="period-th">${esc(p.label)}<small>${esc(p.start)}–${esc(p.end)}</small></td>`;
    for (const w of wdays) {
      const open = g && gradePeriodHasDay(g, p.id, w.wd);
      const rp = reschedResolveOnDate(rec, classId, w.date, w.wd, p.id);
      ...
      if (sid) {
        ...
        const mark = rp.swapped ? `<span class="resched-swapmark" title="由 ${esc(fmtMD(rp.fromDate) + ' ' + DAY_LABELS[rp.day] + periodLabel(rp.period))} 對調而來">🔀</span>` : '';
        const subId = substSubForDate(key, w.date);   // 當日代課
        const teacherLine = subId ? `<small>代 ${esc(teacherName(subId))}<span style="opacity:.75">（原 ${esc(slotTeachersLabel(key))}）</span></small>` : `<small>${esc(slotTeachersLabel(key))}${esc(room)}</small>`;
        html += `<td class="cell"><div class="cell-lesson${rp.swapped ? ' resched-moved' : ''}${subId ? ' resched-subbed' : ''}" style="background:${color};color:${subjTextColor(s, color)}">${mark}${esc(subjectName(sid))}${teacherLine}</div></td>`;
...
// 調課後·某教師·某週課表（依日期式對調後彙整；標🔀）
function reschedTeacherWeekHTML(teacherId, rec, monday) {
  const wdays = weekSchoolDays(monday); const t = teacherById(teacherId);
  const teaches = new Set((t.load || []).map(L => L.classId + '|' + L.subjectId));
  let html = `<div class="print-only" style="text-align:center;font-weight:700;font-size:15px;margin-bottom:6px">${esc((t || {}).name || '')} 教師課表（調課後）</div>`;
  html += `<table class="timetable"><thead><tr><th class="period-th">節次</th>${wdays.map(w => `<th>${DAY_LABELS[w.wd]}<small>${esc(fmtMD(w.date))}</small></th>`).join('')}</tr></thead><tbody>`;
  for (const p of state.settings.periods) {
    if (p.isBreak) { html += `<tr class="break-row"><td colspan="${wdays.length + 1}">${esc(p.label)}</td></tr>`; continue; }
    html += `<tr><td class="period-th">${esc(p.label)}<small>${esc(p.start)}–${esc(p.end)}</small></td>`;
    for (const w of wdays) {
      const hits = [];
      for (const c of state.classes) {
        const rp = reschedResolveOnDate(rec, c.id, w.date, w.wd, p.id);
...
// 組合列印：按週次，各週列出受影響班級 + 受影響教師的當週調課後課表
function reschedPrintHTML(rec) {
  const weeks = reschedWeeks(rec); const classes = reschedAffectedClassIds(rec); const teachers = reschedAffectedTeacherIds(rec);
  const who = rec.teacherId ? teacherName(rec.teacherId) : '';
  let html = '';
  weeks.forEach(monday => {
    const wdays = weekSchoolDays(monday);
    html += `<div class="subst-print-page"><div style="text-align:center;font-weight:800;font-size:17px;margin:6px 0 10px">調課週：${esc(fmtMD(monday))}～${esc(fmtMD(wdays[4].date))}${who ? '　（' + esc(who) + ' 請假）' : ''}${rec.note ? '　' + esc(rec.note) : ''}</div></div>`;
...
function viewSettings() {
  ...
  // v12.42 各設定區塊改為可折疊；展開狀態記在 settingsOpen（key＝data-sg），重繪後保持。
  const grp = (key, title, body) => `<details class="set-card" data-sg="${key}"${settingsOpen[key] ? ' open' : ''}><summary>${title}</summary><div class="set-card-body">${body}</div></details>`;
  const roomsBody = `
    <div style="margin-bottom:10px"><button class="ghost" data-action="add-room">＋ 新增教室</button></div>
  ...
  const outputBody = `
    ...
    <p class="hint" style="color:var(--muted);margin:0 0 12px;font-size:12px">「學校代號」用於「導師線上填課」與「代課／調課線上填報」的檔名／連結，多校共用本系統時各校填不同代號即可區隔，教師開連結也只會看到自己學校的檔。</p>
  ...
  const advancedBody = `
    <section class="set-sub"><h5>🗓️ 學年度轉換</h5>
      <p style="color:var(--muted);margin:4px 0 10px">新學年時一鍵沿用本學年的所有設定（科目／年級／班級／教師配課／教室／協同／領域），只把排課清空重排，免重建。套用前會自動下載備份。</p>
      <button class="btn" data-action="new-school-year">另存為新學年（沿用設定）</button></section>
    <section class="set-sub"><h5>🧑‍🏫 導師線上填課</h5>
      ...
      <button class="btn" data-action="teacher-fill">用 Google 登入並填課</button></section>
    <section class="set-sub"><h5>🧹 只重設本 App</h5>
  ...
  return `
    ...
    ${grp('days', '🗓️ 上課日', `<div>${dayToggles}</div>`)}
    ${grp('periods', '⏰ 節次定義（全校共用）', `
      <div style="margin-bottom:10px"><button class="ghost" data-action="add-period">＋ 新增節次</button></div>
...
function helpModal() {
  ...
  openModal({
    ...
    <div class="help-note">📌 <b>三個重點：</b>①免安裝，開網址就能用；基本操作免登入（只有<b>雲端同步、導師線上填課、代課／調課線上填報</b>才需 Google 登入）。②資料只存<b>你這台裝置</b>的瀏覽器。③換裝置／清瀏覽器前，先用首頁「💾 備份」匯出或開啟雲端同步。</div>
    <p class="help-flow">①科目 ▸ ②年級與班級 ▸ ③教師配課 ▸ ④排課 ▸ 課表輸出</p>
    <p class="help-flow" style="margin-top:6px">定稿後：🔒 鎖定＋導師自編 ▸ 🧑‍🏫 導師線上填課　｜　公布後臨時異動：🔄 代課／🔀 調課</p>
    <p style="color:var(--muted);font-size:13px;margin:2px 0 10px">點各段標題展開／收合。第一次可先照上面兩條流程走一遍。</p>
    ...
    ${sec('① 科目', `<ul>
      <li>建立科目、選<b>底色／文字色</b>，可指定<b>所屬領域</b>（供「領域節數」對照，可留未分類）與<b>預設教室</b>（先在「⚙️ 設定」建好專科教室，這裡指定後該科排課會自動檢查同教室不衝堂；個別配課列可再覆寫）。</li>
      <li>🧩 <b>快速範本</b>：一鍵套用常見科目（國語／數學／體育／藝文…）的整組設定（教學型態／連堂／限制／末節傾向），套用後可再微調——<b>會覆蓋下方進階設定、不改科目名稱</b>。</li>
      ...
      <li>🔒 <b>排課限制</b>＋🪄 <b>自動排課偏好</b>（皆只影響自動排課、手動不受限）：限定上課日／節次、偏好節次、多節分散、不列入教師單日上限等。<b>末節傾向</b>下拉可設「主科盡量不排末節」或「輕科優先排末節」。</li>
      <li>進階規則：<b>同學段排相同節次</b>（1·2／3·4／5·6 各自分組、同日同節，如體育合班）、<b>母語日淨空</b>（教此科的老師在其指定上課日整天只上此科）。</li></ul>`)}
    ...
    ${sec('🔄 代課（分頁「代課」）', `<ul>
      <li><b>用途</b>：某老師請假時<b>找人代上</b>、印出相關課表；<b>不更動原排課時段</b>（只換上課的人）。</li>
      <li><b>流程</b>：＋新增代課 → 選<b>請假老師</b>＋<b>起訖日期</b>（可跨多天／半天／指定節次）→ 調出其課表，點<b>有課的格子</b>指派當節<b>有空堂</b>的老師 → 列印／存 PDF。可多筆、可刪除。</li>
      <li><b>空堂名單看得準</b>：指派時「可代課的老師」是依<b>該日實際課表</b>（已套用當天的調課）判斷誰真的有空，不會把因調課而挪走／挪來的老師算錯。</li>
      <li><b>長假分週指派</b>：以「全部週」為底，某週要換人就切到<b>該週</b>單獨指派（其餘週仍沿用）。</li>
      <li><b>防呆</b>：同一時段已被指派、或請假日期重疊會擋。</li>
      <li>🧑‍🏫 <b>代課老師合併總表</b>：一次看每位代課老師的所有代課。</li></ul>`)}

    ${sec('🔀 調課（分頁「調課」）', `<ul>
      <li><b>用途</b>：課表公布後老師請假，把該時段的課<b>對調</b>到別的時間上（有別於代課的「找人代上」）；同樣<b>不影響已定稿的主課表</b>，只在指定日期套用。</li>
      <li><b>流程</b>：選<b>申請調課老師</b>＋<b>請假起訖日</b> → 系統<b>逐日列出</b>該師當天的課 → 每堂按「安排調課」→ 選<b>週次</b>（可前後幾週）→ 在該週課表格上<b>點目標節次</b>完成對調。可多筆、可編輯／刪除。</li>
      <li><b>連鎖對調</b>：已被前一次調課挪動過的格子，仍可依「<b>最新內容</b>」再對調；系統會依建立順序自動疊加、並擋掉會造成老師衝堂等問題的目標。</li>
      <li><b>與代課互通</b>：逐日列課時，若某堂想改成<b>找人代上</b>可直接按「🔄 改代課」跳去指派；列印時各班／各師課表都會<b>如實反映調課＋代課後</b>的結果。</li>
      <li>🖨️ <b>全校調課後總表</b>：把所有調課合成一份，逐週輸出真正的最終課表。</li></ul>`)}

    ${sec('☁️ 代課／調課線上填報（老師自己填）', `<ul>
      <li><b>用途</b>：把代課、調課開放給老師<b>自己線上填</b>，你再一次收回合併（走 Google Drive、需登入）。</li>
      <li><b>開放</b>：先在「③ 教師」為要參與的老師填<b>Google Email</b>（學校或個人 Gmail 皆可）→ 首頁快速入口「<b>☁️ 線上填報</b>」按開放。系統建<b>一份共用檔</b>、逐位 Email 分享，並給你<b>兩條連結</b>：<b>代課連結</b>（找人代上）與<b>調課連結</b>（對調時段）。</li>
      <li><b>老師端</b>：開連結用自己的 Google 帳號登入 → 選這份「代課／調課填報」檔 → <b>只能新增自己的</b>、不能改別人 → 送出雲端。</li>
      <li><b>多校不混淆</b>：連結帶<b>學校代號</b>（在⚙️設定填），老師開檔時<b>只會看到自己學校</b>的填報檔，也不會看到導師填課檔。</li>
      <li><b>收回</b>：在「☁️ 線上填報」按「收回」——把老師填的<b>代課與調課一起</b>合併回來、用你目前最新課表更新共用檔（你刪過的不會再跑回來），並補分享給新加入的老師。因兩端共用同一份檔，<b>收回一次兩者同步</b>。</li></ul>`)}
...
// v12.42 只回內容（外層由設定頁的可摺疊區塊包住，標題即摺疊標頭）。
function cloudSettingsCard() {
  const busy = cloudBusy ? 'disabled' : '';
  if (!cloudConfigured()) {
    return `<p style="color:var(--warn);margin:0">尚未設定：需先在 Google Cloud Console 建立本 App 專屬的 OAuth 用戶端，並把用戶端 ID 填入程式 <code>GOOGLE_CLIENT_ID</code>。設定後即可一鍵備份／多裝置接續。</p>`;
  }
  if (!cloudState.enabled) {
    return `<p style="color:var(--muted);margin:0 0 10px">連結你的 Google 帳號，把排課資料自動備份到「你自己雲端硬碟」的 App 專屬隱藏資料夾（無伺服器）。可在筆電／桌機間接續，並保留每日歷史版本。</p>
      <button class="btn" data-action="cloud-connect" ${busy}>🔗 連結 Google 雲端備份</button>`;
  }
  ...
  return `${cloudState.email ? `<p style="margin:0 0 4px;color:var(--muted)">帳號：${esc(cloudState.email)}</p>` : ''}
...
const clickHandlers = {
  ...
  'resched-cover-resubst': el => {
    reschedSyncForm();
    const recId = el.dataset.rec, key = el.dataset.key, day = +el.dataset.day, period = el.dataset.period, date = el.dataset.date;
  ...
  'resched-save': () => {
    ...
    rec.swaps.forEach(sw => [{ date: sw.aDate, pos: sw.a }, { date: sw.bDate, pos: sw.b }].forEach(({ date, pos }) => sw.classIds.forEach(cid => {
      ...
      if (subId) conf.push({ className: (classById(cid) || {}).name || '', cell: fmtMD(date) + ' ' + periodLabel(pos.period), subName: teacherName(subId) });
    })));
    if (conf.length) {
      const rows = conf.map(x => `<li>${esc(x.className)}　${esc(x.cell)}：代課 <b>${esc(x.subName)}</b></li>`).join('');
      openModal({ title: '⚠ 與代課安排重疊', body: `<p>下列被對調的課於該日已有<b>代課</b>安排：</p><ul style="margin:8px 0 0;padding-left:20px;line-height:1.7">${rows}</ul><p style="margin-top:10px;color:var(--muted)">調課後這些課會移到新時段，代課老師會跟著移動。確定仍要建立？</p>`, saveLabel: '仍要建立', onSave: () => { commit(); return true; } });
      return;
    }
    commit();
  },
...
function bindGlobal() {
  ...
  document.addEventListener('toggle', e => {
    ...
    else if (d.classList.contains('set-card') && d.dataset.sg) settingsOpen[d.dataset.sg] = d.open;   // v12.42 設定頁摺疊區
  }, true);
